...
import { buildMessage, replacers } from "../features/buildMessage";
...
async function handleNogooning(client: ExtendedClient, interaction: ChatInputCommandInteraction, discordId: string, now: Date, guild: any, interactionMember: GuildMember | undefined, subcommand: string) {
  const { they, their, cat } = replacers;
  ...
  if (endSession) {
    ...
    messageToSend = `<@${discordId}> just ended ${their} nogooning session that started on ${formatDate(existingRecord!.chastityStartDate!)}. If ${they} wish/present to comment, ${they} may do it here. But note that ${they} doesnt have to.`;
  } else {
    await GoonRecord.findOneAndUpdate(
      { discordId },
    ...
    messageToSend = `<@${discordId}> wanted everyone to know that ${they} just/present started a ${subcommand} session until ${formatDate(chastityTheoryEndDate)} :devil: ${cat} ${
      lastDate ? `The last time ${they} goon/past was at ${formatDate(lastDate)}` : ``
...
async function promptGoonConfirmation(client: ExtendedClient, interaction: ChatInputCommandInteraction, discordId: string, theoriticalChastityEnd: Date, guild: any, interactionMember: GuildMember | undefined) {
	const { their, them, cat } = replacers;

	const confirmationRow = new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId("confirm_goon").setLabel("Confirm Goon").setStyle(ButtonStyle.Danger));
	...
	if (interaction.channel instanceof TextChannel || interaction.channel instanceof NewsChannel) {
		...
		collector.on("collect", async (i: any) => {
			if (i.customId === "confirm_goon") {
				const messageToSend = `<@${discordId}> has bravely admitted to failing ${their} no-gooning session early! Let's all support ${them} in their journey !! ${cat} ${cat} \n\n Just fucking KIDDING, ROAST ${them} !! I am quite sure they deserve a punishment`;
...
function generateGoonMessage(discordId: string, lastDate: Date | null, now: Date): string {
	const { they, them, their, cat } = replacers;
	...
	if (lastDate) {
		...
		if (timeDiffInDays < 2) {
			messageToSend = `<@${discordId}> wanted everyone to know that ${they} just/present gooned ${cat} !! ${they} be/present quite horny. ${they} have/present not registered another goon since ${timeDiffInHours} hours ${cat} Quite a horny specimen :3 I think ${they} need/present you to give ${them} a hand ! ${cat}`;
		} else if (timeDiffInDays < 7) {
			messageToSend = `<@${discordId}> wanted everyone to know that ${they} just/present gooned ${cat} !! Do.es ${they} deserve/present a punishment? ${their} last goon report was since ${timeDiffInDays} days ${they} could/present have done better ?!?!`;
		} else {
			messageToSend = `<@${discordId}> just failed ${their} ~${timeDiffInWeeks} week(s) and ${remainingDays} day(s) rampage ${cat}! Must have been a relief!! THAT MUST NOT REMAIN UNPUNISHED ${cat} ${cat}`;
		}
	} else {
		messageToSend = `<@${discordId}> wanted everyone to know that ${they} be/past just gooned ! A new new gooning session has been registered ${cat}`;
	}
