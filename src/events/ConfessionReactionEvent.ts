...
export default {
	...
	async execute(client: ExtendedClient, reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser) {
		...
		if (reaction.emoji.name === "🚩") {
			...
			if (reaction.count && reaction.count >= threshold) {
				...
				if (channel instanceof TextChannel || channel instanceof DMChannel) {
					...
					// Log the deletion
					await channel.send("A confession was removed due to exceeding the report threshold.");
				}
			}
		}
	},
